...
**No fake anything.** Payments are verified on chain before a subscription activates (sender, recipient, amount, success — a tx hash can only be used once). Reports are generated by a real LLM and stored in SQLite. GitHub stats are fetched live. Stats on the site are database counts, not marketing numbers.
...
├── lib/
│   ├── agent-runtime.ts   LLM runtime — real reports, real answers
...
| | |
|---|---|
| 🧠 **A real runtime** | `generate_report` · `ask` · `get_latest_reports` · `get_status` — LLM-backed, not canned |
