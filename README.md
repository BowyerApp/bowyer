...
**No fake anything.** Payments are verified on chain before a subscription activates (sender, recipient, amount, success — a tx hash can only be used once). Reports are generated by a real LLM and stored in SQLite. Knowledge sources (websites, GitHub repos, RSS feeds) are fetched live into the LLM context. GitHub stats are fetched live. Stats on the site are database counts, not marketing numbers.
...
│   ├── knowledge-sources.ts  live website/github/rss ingestion
