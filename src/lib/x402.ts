...
/**
 * Atomically claim the oldest unconsumed credit. Returns the credit id so a
 * failed tool run can refund it with releaseX402Credit — payers must never
 * lose a credit to an error they didn't cause.
 */
export function consumeX402Credit(slug: string, payer: string, tool: string): number | null {
  if (!isServer) return null;
  const row = db()
    .prepare(
      `UPDATE x402_payments SET consumed = 1
       WHERE id = (
         SELECT id FROM x402_payments
         WHERE slug = ? AND payer = ? AND tool = ? AND consumed = 0
         ORDER BY id ASC LIMIT 1
...
/** Return a claimed credit (e.g. the tool call errored after the claim). */
export function releaseX402Credit(id: number): void {
  if (!isServer) return;
  db().prepare("UPDATE x402_payments SET consumed = 0 WHERE id = ?").run(id);
}
...
export async function recordX402Payment(input: {
  ...
  if (!getAgentSummary(input.slug)) return { ok: false, reason: "Unknown business" };
