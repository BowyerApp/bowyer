...
export async function POST(
  ...
  if (needsAccessCheck) {
    ...
    if (isPaid && subscribed) {
      ...
      if (paymentTx) {
        ...
        if (!recorded.ok && !hasUnconsumedX402Credit(slug, wallet, toolName)) {
      ...
      x402CreditId = consumeX402Credit(slug, wallet, toolName);
      if (x402CreditId == null && isPaid) {
        const requirement = buildX402Requirement(slug, toolName);
        return NextResponse.json(
          {
            jsonrpc: "2.0",
  ...
  if ("error" in response && !("result" in response)) {
    // The paid call never ran — give the credit back.
