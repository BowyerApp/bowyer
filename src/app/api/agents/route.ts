...
export async function POST(req: Request) {
  ...
  // Every new business gets its own three.ws 3D body. Forging takes ~1-3
  // minutes, so it runs after the response — the agent is live immediately
  // and the robot appears when ready. Uploaded GLBs always win over this.
  void import("@/lib/agent-forge")
    .then(({ forgeAgentModel }) => forgeAgentModel({ slug, name, tagline, category }))
    .catch((err) => console.error(`[forge] launch forge failed for ${slug}:`, err));
